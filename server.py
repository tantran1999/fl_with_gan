...
from federated_learning.worker_selection import SelectSingleAttacker, normal_workers
from client import Client
import random
from experiment_result import save_images_result
...
def train_subset_of_clients(epoch, args, clients, poisoned_workers):
    ...
    if epoch == 1000:
        random_workers = SelectSingleAttacker().select_round_workers(
    ...
    for client_idx in random_workers:
        args.get_logger().info("Training epoch #{} on client #{} | Poisoned: #{}", str(epoch), str(clients[client_idx].get_client_index()), str(clients[client_idx].get_poisoned()))
        clients[client_idx].train(epoch)

    # Averaging model's parameters from chosen clients and attacker
    args.get_logger().info("Averaging client parameters")
    parameters = [clients[client_idx].get_nn_parameters() for client_idx in random_workers]

    # if epoch == 100:
    #     for _ in range(10):
    #         parameters.append(clients[poisoned_workers[0]].get_nn_parameters())

    # Train GAN on attacker
    # if epoch < 100:
    #     clients[poisoned_workers[0]].train_GAN(epoch)
...
def create_clients(args, train_data_loaders, test_data_loader, poisoned_workers):
    ...
    for idx in range(args.get_num_workers()):
        if idx in poisoned_workers:
            poisoned = True
        else:
            poisoned = False
        client = Client(args, idx, train_data_loaders[idx], test_data_loader, poisoned)
...
def run_exp(replacement_method, num_poisoned_workers, KWARGS, client_selection_strategy, idx, exp_name):
    ...
    save_images_result(exp_path='experiment_mnist/%s/exp_%s' % (exp_name, idx), csv_path="%s_results.csv" % idx)
